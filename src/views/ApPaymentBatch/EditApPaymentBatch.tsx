...
export default function EditApPaymentBatch() {
  ...
  useEffect(() => {
    const loadBankAccounts = async () => {
      try {
        ...
        console.error("Không tải được danh sách tài khoản ngân hàng chi trả.");
      }
    };
    loadBankAccounts();
  }, []);
      ...
      try {
        ...
        if (res.success && res.data) {
          form.setValues({
            ...
            bankAccountId: String(res.data.bankAccountId),
            paymentDate: res.data.paymentDate ? res.data.paymentDate.substring(0, 10) : "",
  ...
  const handleSubmit = async (values: typeof form.values) => {
    if (!id) return;
    try {
      const res = await apPaymentBatchService.update(Number(id), {
      ...
      if (res.success) {
        NotificationExtension.Success("Cập nhật lô thanh toán thành công!");
        navigate("/ApPaymentBatch/ApPaymentBatchList");
      } else {
  ...
  return (
    <Box>
      ...
      <Card withBorder shadow="sm" radius="md" padding="xl">
        <Title order={3} mb="xs">Cập nhật Lô thanh toán hàng loạt</Title>
        <Text size="sm" c="dimmed" mb="lg">Mã lô thanh toán (ID): <b>{id}</b></Text>
        <Divider mb="xl" />
