...
import { Box, Card, Title, TextInput, Select, Group, Button, Grid, Divider, NumberInput, Table, ActionIcon, Text, Center, Loader } from "@mantine/core";
...
import { ApInvoiceLine } from "../../model/ApInvoiceMasterModel";
...
export default function EditApInvoice() {
    ...
    const loadInvoiceData = async () => {
      ...
      try {
        ...
        if (invRes.success && invRes.data) {
          ...
          form.setValues({
            setOfBooksId: inv.setOfBooksId,
            orgId: inv.orgId,
            batchId: inv.batchId ? String(inv.batchId) : "",
            vendorId: String(inv.vendorId),
            vendorSiteId: String(inv.vendorSiteId),
            ...
            baseAmount: inv.baseAmount || 0,
            invoiceTypeLookupCode: inv.invoiceTypeLookupCode || "STANDARD",
            description: inv.description || "",
            termsId: inv.termsId ? String(inv.termsId) : "",
            termsDate: inv.termsDate ? inv.termsDate.substring(0, 10) : "",
            paymentCurrencyCode: inv.paymentCurrencyCode || "VND",
            paymentMethodLookupCode: inv.paymentMethodLookupCode || "WIRE",
            acctsPayCodeCombinationId: inv.acctsPayCodeCombinationId || 33101,
            poNumber: inv.poNumber || ""
          });

          // Load lines
          const lineList = await apInvoiceMasterService.getLines(Number(id));
          const mappedLines: LineFormItem[] = lineList.map((l) => ({
            ...
            price: l.price || 0,
            amount: l.amount || 0,
            distCodeCombinationId: l.distCodeCombinationId || 62701,
            lineTypeLookupCode: l.lineTypeLookupCode || "ITEM",
            vatTaxCode: l.vatTaxCode || "VAT10",
            vatTaxAmount: l.vatTaxAmount || 0,
            description: l.description || ""
          }));
          setLines(mappedLines);
        } else {
          NotificationExtension.Fails(invRes.message || "Không tải được hóa đơn.");
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    try {
      ...
      const res = await apInvoiceMasterService.update(Number(id), {
        setOfBooksId: Number(values.setOfBooksId),
        orgId: Number(values.orgId),
      ...
      if (res.success) {
        NotificationExtension.Success("Cập nhật hóa đơn mua hàng thành công!");
        navigate("/ApInvoice/ApInvoiceList");
      } else {
        NotificationExtension.Fails(res.message || "Lỗi khi lưu hóa đơn.");
      }
    } catch {
      NotificationExtension.Fails("Không thể lưu thay đổi hóa đơn.");
    }
  };
  ...
  return (
    <Box>
      ...
      <Card withBorder shadow="sm" radius="md" padding="xl" mb="xl">
        <Title order={3} mb="xs">Cập nhật Hóa đơn Mua hàng</Title>
        <Text size="sm" c="dimmed" mb="lg">Mã hóa đơn (ID): <b>{id}</b></Text>
        <Divider mb="xl" />
