...
import { ApVendorSite, ApVendor } from "../../model/ApVendorMasterModel";
import BreadCrumb from "../../_base/component/_layout/_breadcrumb";

export default function ApVendorSiteList() {
  const navigate = useNavigate();
  const headerRef = useRef<HTMLDivElement>(null);

  const [data, setData] = useState<ApVendorSite[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await apVendorSiteMasterService.getList({
  ...
  const columns = useMemo<MRT_ColumnDef<ApVendorSite>[]>(
    () => [
      {
        accessorKey: "vendorSiteCode",
        header: "Mã chi nhánh (Site Code)",
        size: 180,
        Cell: ({ row }) => (
          <span
            style={{ cursor: "pointer", color: "#228be6", fontWeight: 600 }}
            onClick={() => navigate(`/ApVendorSite/Detail/${row.original.vendorSiteId}`)}
          >
            {row.original.vendorSiteCode}
          </span>
        )
      },
      {
        accessorKey: "vendorName",
        header: "Thuộc Nhà cung cấp",
        size: 240,
      },
      {
    ...
    <Box>
      <div style={{ border: "1px solid #dee2e6", padding: "10px", backgroundColor: "#fff" }} ref={headerRef}>
        <Flex justify="space-between" align="center" mb="10">
          <BreadCrumb />
          <Title order={4}>Chi nhánh giao dịch nhà cung cấp</Title>
        </Flex>
        ...
        <form onSubmit={formSearch.onSubmit(handleSearch)}>
          <Grid align="flex-end">
            <Grid.Col span={{ base: 12, md: 3 }}>
              <TextInput
                placeholder="Tìm mã, địa chỉ, ngân hàng..."
            ...
            <Grid.Col span={{ base: 12, md: 3 }}>
              <Flex gap="sm" justify="flex-end">
                ...
                <Button
                  ...
                  color="blue"
                  variant="outline"
                  onClick={() => navigate("/ApVendorSite/Create")}
                >
                  Thêm chi nhánh
                </Button>
              </Flex>
            ...
            <Flex gap="xs" justify="center">
              ...
              <Tooltip label="Chỉnh sửa">
                <ActionIcon
                  variant="light"
                  color="yellow"
                  onClick={() => navigate(`/ApVendorSite/Edit/${row.original.vendorSiteId}`)}
                >
                  <IconEdit size={18} />
                </ActionIcon>
              </Tooltip>
              <Tooltip label="Xóa">
                <ActionIcon
                  variant="light"
                  color="red"
                  onClick={() => navigate(`/ApVendorSite/Delete/${row.original.vendorSiteId}`)}
                >
                  <IconTrash size={18} />
                </ActionIcon>
              </Tooltip>
