...
export default function EditApInvoicePayment() {
      ...
      try {
        ...
        if (res.success && res.data) {
          form.setValues({
            ...
            accountingDate: res.data.accountingDate ? res.data.accountingDate.substring(0, 10) : "",
            periodName: res.data.periodName,
            amount: res.data.amount || 0,
            ccidDr: res.data.ccidDr || 33101,
            ccidCr: res.data.ccidCr || 112101,
            reversalFlag: res.data.reversalFlag || "N",
            currencyCode: res.data.currencyCode,
            exchangeRate: res.data.exchangeRate || 1,
            invoiceAmount: res.data.invoiceAmount || 0,
            invoiceBaseAmount: res.data.invoiceBaseAmount || 0,
            paymentAmount: res.data.paymentAmount || 0,
            paymentBaseAmount: res.data.paymentBaseAmount || 0,
        ...
        NotificationExtension.Fails("Lỗi khi tải thông tin phân bổ.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (values: typeof form.values) => {
    if (!id) return;
    try {
      const res = await apInvoicePaymentService.update(Number(id), {
  ...
  return (
    <Box>
      ...
      <Card withBorder shadow="sm" radius="md" padding="xl">
        <Title order={3} mb="xs">Cập nhật Phân bổ Thanh toán Hóa đơn</Title>
        <Text size="sm" c="dimmed" mb="lg">Mã phân bổ (ID): <b>{id}</b></Text>
        <Divider mb="xl" />
