...
export default function CreateARInvoice() {
  ...
  // Load Setup references
  useEffect(() => {
    const loadSetup = async () => {
      const [srcRes, typeRes, termRes, accRes] = await Promise.all([
      ...
      lines: [
        {
          ...
          uom: "CAI",
          quantityOrdered: 1,
          unitStandardPrice: 1000000,
          unitSellingPrice: 1000000,
          lineAmount: 1000000,
          taxRate: 10,
          taxAmount: 100000,
          discountAmount: 0,
          totalAmount: 1100000,
          revenueAccount: 511100,
          taxAccount: 333110
        }
      ]
    },
    validate: {
      soldToCustomerId: (value: string) => (!value ? "Khách hàng là bắt buộc" : null),
      lines: {
        itemCode: (value: string) => (!value ? "Mã hàng bắt buộc" : null),
        quantityOrdered: (value: number) => (value <= 0 ? "Số lượng phải > 0" : null),
        unitStandardPrice: (value: number) => (value < 0 ? "Đơn giá không được âm" : null)
      }
    }
  });
  ...
  const handleSubmit = async (values: typeof form.values, status: string) => {
    ...
    const res = await arBillingService.create(req as any);
    if (res.success) {
      NotificationExtension.Success(status === "COMPLETE" ? "Đã lưu và Hoàn tất hóa đơn!" : "Đã lưu nháp hóa đơn!");
        ...
        <Grid.Col span={12}>
          <Card withBorder shadow="sm" radius="sm">
            ...
            <Grid>
              ...
              <Grid.Col span={{ base: 12, md: 4 }}>
                <TextInput label="Số hóa đơn (Được cấp sau khi lưu)" disabled placeholder="Hệ thống tự cấp..." />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 4 }}>
              ...
              <Grid.Col span={{ base: 12, md: 4 }}>
                <TextInput label="Số hợp đồng/PO tham chiếu" placeholder="Số tham chiếu..." {...form.getInputProps("referenceNumber")} />
              </Grid.Col>
        ...
        <Grid.Col span={12}>
          <Card withBorder shadow="sm" radius="sm">
            ...
            <Grid>
              ...
              <Grid.Col span={{ base: 12, md: 6 }}>
                <TextInput label="Mã số thuế" placeholder="MST tự điền..." disabled value={form.values.soldToCustomerId ? "010998822" + form.values.soldToCustomerId : ""} />
              </Grid.Col>
            </Grid>
          </Card>
        </Grid.Col>

        {/* SECTION 3: LINES TABLE */}
        <Grid.Col span={12}>
          <Card withBorder shadow="sm" radius="sm" style={{ overflowX: "auto" }}>
            <Flex justify="space-between" align="center" mb="sm">
              <Title order={5} c="indigo">Chi tiết dòng hóa đơn (Invoice Lines)</Title>
              <Button leftSection={<IconPlus size={14} />} color="teal" variant="outline" onClick={addLine}>
                Thêm dòng
              </Button>
            </Flex>
            ...
            <Table striped withTableBorder withColumnBorders>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th style={{ width: 160 }}>Tài khoản DT</Table.Th>
              ...
              <Table.Tbody>
                {form.values.lines.map((item, idx) => (
                  <Table.Tr key={idx}>
                    ...
                    <Table.Td>
                      <TextInput placeholder="Mã hàng..." value={item.itemCode} onChange={(e) => handleLineChange(idx, "itemCode", e.target.value)} />
                    </Table.Td>
                    <Table.Td>
                      <TextInput placeholder="Tên sản phẩm..." value={item.itemName} onChange={(e) => handleLineChange(idx, "itemName", e.target.value)} />
                    </Table.Td>
                    <Table.Td>
                    ...
                    <Table.Td>
                      <Select
                        ...
                        value={String(item.revenueAccount)}
                        onChange={(val) => handleLineChange(idx, "revenueAccount", Number(val))}
                      />
                    </Table.Td>
                    <Table.Td>
