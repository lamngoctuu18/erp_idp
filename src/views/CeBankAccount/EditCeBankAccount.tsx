...
export default function EditCeBankAccount() {
      ...
      try {
        ...
        if (res.success && res.data) {
          form.setValues({
            ...
            branchId: res.data.branchId ? String(res.data.branchId) : "",
            legalEntityId: res.data.legalEntityId,
            currencyCode: res.data.currencyCode,
            alternateAccountName: res.data.alternateAccountName || "",
            multiCurrencyAllowedFlag: res.data.multiCurrencyAllowedFlag || "N",
  ...
  const handleSubmit = async (values: typeof form.values) => {
    if (!id) return;
    try {
      const res = await ceBankAccountService.update(Number(id), {
      ...
      if (res.success) {
        NotificationExtension.Success("Cập nhật thông tin tài khoản thành công!");
        navigate("/CeBankAccount/CeBankAccountList");
      } else {
  ...
  return (
    <Box>
      ...
      <Card withBorder shadow="sm" radius="md" padding="xl">
        <Title order={3} mb="xs">Cập nhật thông tin Tài khoản Ngân hàng</Title>
        <Text size="sm" c="dimmed" mb="lg">Mã số tài khoản (ID): <b>{id}</b></Text>
        <Divider mb="xl" />
