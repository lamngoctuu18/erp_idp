...
export default function CreateApPaymentSchedule() {
  ...
  const handleSubmit = async (values: typeof form.values) => {
    try {
      const res = await apPaymentScheduleService.create({
      ...
      if (res.success) {
        NotificationExtension.Success("Thêm mới lịch trả nợ thành công!");
  ...
  return (
    <Box>
      ...
      <Card withBorder shadow="sm" radius="md" padding="xl">
        <Title order={3} mb="xs">Thêm mới Lịch Trả Nợ dự kiến (AP Payment Schedule Entry)</Title>
        <Divider mb="xl" />
